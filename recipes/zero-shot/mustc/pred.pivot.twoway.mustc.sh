...
mkdir $OUTDIR/$MODEL/mustc/pivot/twoway -p
...
for src in $LAN; do
    for tgt in $LAN; do
        ...
        if [[ $src != $tgt ]] && [[ $src != en ]] && [[ $tgt != en ]]; then
            ...
            ln -s -f $DATADIR/mustc/prepro_20000_subwordnmt/twoway/test/$sl-$tl.s  $OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotin.s 
            
            pred_src=$OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotin.s
            out=$OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotin.t
            ...
            ln -s -f $OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotin.t $OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotout.s

            pred_src=$OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotout.s
            out=$OUTDIR/$MODEL/mustc/pivot/twoway/${src}-$pvt-${tgt}.real.pivotout.t
            ...
            cat $out.pt | sacrebleu $DATADIR/mustc/twoway/tst-COMMON/$src-$tgt.t > $out.res
            cat $out.res
        fi
    done
done
